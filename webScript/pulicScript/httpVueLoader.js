(function umd (root, factory) {
  ...
  httpVueLoader.register = function (app, url) {
    ...
    app.component(comp.name, Vue.defineAsyncComponent(httpVueLoader.load(comp.url)))
  }
      ...
      beforeCreate: function () {
        ...
        for (const componentName in components) {
          if (typeof (components[componentName]) === 'string' && components[componentName].substr(0, 4) === 'url:') {
            ...
            if (isNaN(componentName)) { components[componentName] = Vue.defineAsyncComponent(httpVueLoader.load(componentURL, componentName)) } else { components[componentName] = Vue.component(comp.name, Vue.defineAsyncComponent(httpVueLoader.load(componentURL, comp.name))) }
          }
        }
      }
    })
